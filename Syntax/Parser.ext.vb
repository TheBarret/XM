...
Namespace Syntax
    Partial Public Class Parser
        ...
        Private Function ParseHexadecimal(signed As Boolean, Optional op As Tokens = Tokens.T_Null) As Expression
            Try
                If (signed And op = Tokens.T_Minus) Then
                    Return New TInteger(Integer.Parse(Me.Current.Value.StripHexPrefix, Env.Hexadecimal) * -1)
                End If
                Return New TInteger(Integer.Parse(Me.Current.Value.StripHexPrefix, Env.Hexadecimal) * 1)
            Finally
                Me.NextToken()
        ...
        Private Function ParseArray() As Expression
            Return New TArray(Me.ParseArrayValues)
        End Function
        ...
        Private Function ParseFunction() As Expression
            ...
            If (Me.Current.Type = Tokens.T_Identifier) Then
                Return New TFunction(CType(Me.ParseIdentifier, TIdentifier), Me.ParseTuples, Me.ParseBraceBlock)
            Else
                Return New TFunction(New TIdentifier(Env.RandomName(8, "func_")), Me.ParseTuples, Me.ParseBraceBlock)
            End If
        End Function
        Private Function ParseBraceBlock() As List(Of Expression)
            ...
            Return body
        End Function
        Private Function ParseArrayValues() As List(Of Expression)
            Dim params As New List(Of Expression)
            Me.Match(Tokens.T_BracketOpen)
            ...
            Return params
        End Function
        Private Function ParseTuples() As List(Of Expression)
            Dim params As New List(Of Expression)
